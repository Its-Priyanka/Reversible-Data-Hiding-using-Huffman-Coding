img2 = imread('lsb_watermarked.bmp');
[pixelCounts, grayLevels] = imhist(img2);
m=max(pixelCounts);
...
imshow(Z,[])
isequal(message,arr) 
imwrite(img2,'stego.bmp');
imshow(img2)                        %Check if hidden image and image after extraction are equal